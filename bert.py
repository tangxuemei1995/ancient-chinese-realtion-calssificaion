    ...
    if config.test:
        ...
        for i in range(1, len(test_precision)):
            if test_precision[i] == 0 and test_recall[i] == 0:
                ...
                test_p.append((count_total[i]/len(y_pre))*test_precision[i])

                test_r.append((count_total[i]/len(y_pre))*test_recall[i])

                test_f1s.append((count_total[i]/len(y_pre))*(2*test_precision[i]*test_recall[i])/(test_precision[i]+test_recall[i]))
        ...
        f.write('\n')
        f2 = open('./data/' + config.task_name +'/test.tsv','r',encoding='utf-8')
